...
contract FlareSystemCalculator is Governed, AddressUpdatable {
  ...
  ) external onlyVoterRegistry returns (uint256 _registrationWeight) {
  ...
  function calculateBurnFactorPPM(uint24 _rewardEpochId, address _voter) external view returns (uint256) {
    (uint64 startTs, uint64 startBlock, uint64 endTs, uint64 endBlock) = flareSystemManager.getSigningPolicySignInfo(
      _rewardEpochId
    );
