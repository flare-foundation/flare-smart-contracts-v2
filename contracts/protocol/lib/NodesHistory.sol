...
library NodesHistory {
    ...
    function cleanupOldCheckpoints(
        ...
        while (
            index < endIndex &&
            _self.checkpoints[index + 1].nodeIds[0].fromBlock <=
            _cleanupBlockNumber
        ) {
    ...
    function _updateNodeIds(
        ...
        if (i < length) {
            ...
            uint256 newlength = _appendNodeId(
                _cp,
                _nodeId,
                length,
                _maxNodeIds
    ...
    function _findGreatestBlockLessThan(
        ...
        if (
            ...
            _blockNumber >=
            _self.checkpoints[historyCount - 1].nodeIds[0].fromBlock
        ) {
        ...
        if (_blockNumber < _self.checkpoints[startIndex].nodeIds[0].fromBlock) {
            ...
            require(
                startIndex == 0,
                "NodesHistory: reading from cleaned-up block"
            );
            return (false, 0);
        }
        ...
        return (
            true,
            _binarySearchGreatestBlockLessThan(
                _self.checkpoints,
                startIndex,
                historyCount,
                _blockNumber
