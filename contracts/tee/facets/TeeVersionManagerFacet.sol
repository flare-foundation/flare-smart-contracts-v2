...
/**
 * @title TeeVersionManagerFacet
 * @notice Facet for managing TEE upgrade versions and signing.
 */
contract TeeVersionManagerFacet is ITeeVersionManagerFacet {

    modifier onlyValidTeeUpgradeId(uint256 _teeUpgradeId) {
        require(
            _teeUpgradeId < TeeVersionManager.getState().teeUpgrades.length,
            InvalidUpgradeId()
        );
        _;
    }

    /// @inheritdoc ITeeVersionManagerFacet
        ...
        returns (uint256 _teeUpgradeId)
    {
        ...
        TeeVersionManager.State storage s = TeeVersionManager.getState();
        _teeUpgradeId = s.teeUpgrades.length;
        TeeVersionManager.TeeUpgrade storage upgrade = s.teeUpgrades.push();
    ...
    {
        TeeVersionManager.State storage s = TeeVersionManager.getState();
        TeeVersionManager.TeeUpgrade storage teeUpgrade = s.teeUpgrades[_teeUpgradeId];
        TeeExtensionRegistry.checkOnlyExtensionOwner(teeUpgrade.extensionId);
        ...
        for (uint256 i = 0; i < _upgradePaths.length; i++) {
            ...
            TeeVersionManager.TeeUpgradePathState storage upgradePathState =
        ...
        onlyValidTeeUpgradeId(_teeUpgradeId)
    {
        ...
        bool sourceSig = TeeGovernance.isTeeGovernanceSigner(extensionId, sourceTeeGovernanceHash, signer);
        if (sourceSig) {
        ...
        bool targetSig = TeeGovernance.isTeeGovernanceSigner(extensionId, targetTeeGovernanceHash, signer);
        if (targetSig) {
        ...
        uint64 sourceThreshold =
            TeeGovernance.getTeeGovernanceThreshold(extensionId, sourceTeeGovernanceHash);
        uint64 targetThreshold =
            TeeGovernance.getTeeGovernanceThreshold(extensionId, targetTeeGovernanceHash);
    ...
    function isTeeUpgradePathValid(
        ...
        returns (bool)
    {
        return TeeVersionManager.isTeeUpgradePathValid(
    ...
    function isTeeUpgradeFinalized(
        ...
        returns (bool)
    {
        return TeeVersionManager.getState().teeUpgrades[_teeUpgradeId].messageHash != bytes32(0);
    }
    ...
    function isTeeUpgradeSigned(
        ...
        returns (bool)
    {
        return TeeVersionManager.isTeeUpgradeSigned(_teeUpgradeId);
    }
    ...
    function getTeeUpgradesCount()
        ...
        returns (uint256)
    {
        return TeeVersionManager.getState().teeUpgrades.length;
    }
    ...
    {
        TeeVersionManager.TeeUpgrade storage teeUpgrade =
            TeeVersionManager.getState().teeUpgrades[_teeUpgradeId];
    ...
    {
        TeeVersionManager.TeeUpgradePathState[] storage teeUpgradePaths =
            TeeVersionManager.getState().teeUpgrades[_teeUpgradeId].upgradePaths;
