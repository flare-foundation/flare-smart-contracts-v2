...
library TeeVersionManager {
    ...
    /// @custom:storage-location erc7201:tee.TeeVersionManager.State
    struct State {
        TeeUpgrade[] teeUpgrades;
    }

    bytes32 internal constant STATE_POSITION = keccak256(
        abi.encode(uint256(keccak256("tee.TeeVersionManager.State")) - 1)
        ...
        returns (bool)
    {
        ...
        require(_teeUpgradeId < s.teeUpgrades.length, ITeeVersionManagerFacet.InvalidUpgradeId());
        ...
        require(teeUpgrade.messageHash != bytes32(0), ITeeVersionManagerFacet.UpgradeNotFinalized());
        bytes32 sourceVersionHash = keccak256(
            abi.encode(ITeeVersionManagerFacet.TeeNodeVersion(_sourceCodeHash, _sourcePlatform))
        );
        bytes32 targetVersionHash = keccak256(
            abi.encode(ITeeVersionManagerFacet.TeeNodeVersion(_targetCodeHash, _targetPlatform))
        );
