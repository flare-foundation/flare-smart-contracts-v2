...
// Only the SIGNATURES (v,r,s) are symbolic; ecrecover is the uninterpreted recovery function E, so the
...
// the protocol-1 parser (both votingRoundId and isSecureRandom must be 0). For protocolId==3 the message uses
...
// "Wrong sign policy reward epoch"/"Message too old"/"Delayed sign policy" guards pass and the
// messageRewardEpochId > rewardEpochId threshold-increase block is never entered.
contract RelayReturnDiscriminatorFV is RelayTestBase {
    bytes internal policy;
    bytes32 internal constant ROOT = keccak256("fv-root"); // concrete, non-zero; only matters for pid > 1
    ...
    // Custom-signature return shape: protocolId==1 success => returndata length is EXACTLY 35.
    // EXPECT: PASS. This underpins _verifyCustomSignature's exact-length check.
    ...
    // Reachability control: acceptance on the protocolId==1 path MUST be reachable.
    // EXPECT: COUNTEREXAMPLE (an `ok == true` witness). If this PASSES, the return-shape proof is vacuous.
    ...
    // Mode-2 return shape: protocolId==3 success => returndata length is EXACTLY 0.
    // EXPECT: PASS. (Anchors `return(0,0)` on the non-random-protocol branch, 3 != 2.)
    ...
    // Load-bearing discriminator fact: protocolId==3 success => length is NOT 35
    // (so _verifyCustomSignature's 35-byte length check cannot be satisfied by a Mode-2 message).
    // Subsumed by the exact-zero return-shape proof but stated explicitly to mirror
    // _verifyCustomSignature's discriminator usage.
    // EXPECT: PASS.
    ...
    // Reachability control: acceptance on the protocolId==3 path MUST be reachable.
    // EXPECT: COUNTEREXAMPLE (an `ok == true` witness). If this PASSES, both Mode-2 return proofs are vacuous.
