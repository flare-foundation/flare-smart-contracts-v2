...
contract EntityManagerTest is Test {
    ...
    function testRegisterNodeIdWithNodePossessionVerification() public {
        ...
        mockNodePossessionVerification.setVoterAndNodeId(user1, nodeId1);
        vm.prank(governance);
        entityManager.setNodePossessionVerifier(mockNodePossessionVerification);

        bytes memory certificateRaw = mockNodePossessionVerification.CERTIFICATE_RAW_TEST();
        bytes memory signature = mockNodePossessionVerification.SIGNATURE_TEST();

        vm.prank(user1);
        vm.expectRevert("node possession verification failed");
        entityManager.registerNodeId(nodeId1, "", "");
    ...
    function testSetMaxNodeIdsPerEntityZero() public {
        ...
        vm.expectRevert("node id registration not enabled");
        entityManager.registerNodeId(nodeId1, "", "");
    ...
    function testRegisterPublicKeyRevertVerificationFailed2() public {
        ...
        entityManager.registerPublicKey(publicKey1, publicKey2, abi.encode(2, 2, 3));
    }

    function testRegisterPublicKeyRevertPublicKeyRegistrationNotEnabled() public {
        assertEq(address(entityManager.publicKeyVerifier()), address(mockPublicKeyVerification));

        vm.prank(governance);
        entityManager.setPublicKeyVerifier(IIPublicKeyVerifier(address(0)));
        assertEq(address(entityManager.publicKeyVerifier()), address(0));
