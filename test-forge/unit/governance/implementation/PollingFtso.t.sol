...
contract PollingFtsoTest is Test {
    ...
    function testProposeByProxy() public {
        uint256 currentRewardEpoch = 10;
        _mockGetCurrentRewardEpochId(currentRewardEpoch);
        _setVoters(4, currentRewardEpoch, true);
        _mockGetWeightsSums(currentRewardEpoch, 1000);

        // set proxy voter for first registered voter
        vm.prank(voters[0]);
        pollingFtso.setProxyVoter(proxyVoter);
        _mockIsVoterRegistered(proxyVoter, currentRewardEpoch, false);

        // proxy creates a proposal
        vm.deal(proxyVoter, 1 ether);
        vm.prank(proxyVoter);
        pollingFtso.propose{ value: 100} ("proposal1");
        (, , address proposerAddr, , , , , ) = pollingFtso.getProposalInfo(1);
    ...
    function testCancelByProxy() public {
        ...
        vm.prank(proposer);
        pollingFtso.setProxyVoter(proxyVoter);
        _mockIsVoterRegistered(proxyVoter, 10, false);
    ...
    function testVoteByProxy() public {
        ...
        emit VoteCast(voters[0], 1, 1, 100, 0);
        pollingFtso.castVote(1, 1);
        assertEq(pollingFtso.hasVoted(1, voters[0]), true);

        address proxy2 = makeAddr("proxy2");
        _mockIsVoterRegistered(voters[4], 10, false);
        _mockIsVoterRegistered(proxy2, 10, false);
        vm.prank(voters[4]);
        pollingFtso.setProxyVoter(proxy2);

        // voters[4] is not registered
        vm.prank(proxy2);
        vm.expectRevert("address is not eligible to cast a vote");
        pollingFtso.castVote(1, 1);
        ...
        emit VoteCast(voters[1], 1, 1, 300, 0);
        pollingFtso.castVote(1, 1);
        assertEq(pollingFtso.hasVoted(1, voters[1]), true);
    ...
    function testProxyVotesInItsOwnName() public {
        testPropose();
        _setVoters(3, 10, true);
        _mockIsVoterRegistered(proxyVoter, 10, true);
        vm.mockCall(
            mockVoterRegistry,
        ...
        vm.prank(proxyVoter);
        vm.expectEmit();
        emit VoteCast(proxyVoter, 1, 1, 400, 0);
        pollingFtso.castVote(1, 1);
        assertEq(pollingFtso.hasVoted(1, proxyVoter), true);

        // voters[0] votes
        vm.prank(voters[0]);
        vm.expectEmit();
        emit VoteCast(voters[0], 1, 1, 500, 0);
        pollingFtso.castVote(1, 1);
        assertEq(pollingFtso.hasVoted(1, voters[0]), true);
        ...
        vm.prank(voters[1]);
        vm.expectEmit();
        emit VoteCast(voters[1], 1, 1, 700, 0);
        pollingFtso.castVote(1, 1);
        assertEq(pollingFtso.hasVoted(1, voters[1]), true);
    ...
    function testCanProposeAndCanVote() public {
        ...
        assertEq(pollingFtso.canPropose(voters[0]), true);
        assertEq(pollingFtso.canPropose(proxyVoter), true);
        assertEq(pollingFtso.canPropose(voters[1]), true);
        assertEq(pollingFtso.canPropose(voters[2]), true);
        assertEq(pollingFtso.canPropose(voters[3]), true);
        assertEq(pollingFtso.canPropose(voters[4]), false);
        assertEq(pollingFtso.canPropose(proxy2), false);
        assertEq(pollingFtso.canPropose(maintainer), true);
        ...
        assertEq(pollingFtso.canVote(voters[0], 1), true);
        assertEq(pollingFtso.canVote(proxyVoter, 1), true);
        assertEq(pollingFtso.canVote(voters[1], 1), true);
        assertEq(pollingFtso.canVote(voters[2], 1), true);
        assertEq(pollingFtso.canVote(voters[3], 1), true);
        assertEq(pollingFtso.canVote(voters[4], 1), false);
        assertEq(pollingFtso.canVote(proxy2, 1), false);
        assertEq(pollingFtso.canVote(maintainer, 1), false);
    }
