...
contract TeeExtensionRegistryFacetTest is Test {
    ...
    function setUp() public {
        ...
        _setupTeeMachine(teeIds[1], extensionId, url, ITeeMachineRegistryFacet.TeeStatus.PRODUCTION);
    ...
    function testSendInstructionsRevertOperationTypeEmpty() public {
        ...
        vm.expectRevert(ITeeExtensionRegistryFacet.OperationTypeEmpty.selector);
        flareTeeManager.sendInstructions(
            teeIds,
            ITeeExtensionRegistryFacet.TeeInstructionParams(
    ...
    function testSendInstructionsRevertSystemOpTypeNotAllowed() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ITeeExtensionRegistryFacet.SystemOpTypeNotAllowed.selector,
                opType
            )
        );
        flareTeeManager.sendInstructions(
    ...
    function testSendInstructionsRevertTeeMachineNotAvailable() public {
        testRegister();
        _setupTeeMachine(teeIds[0], extensionId, url, ITeeMachineRegistryFacet.TeeStatus.PAUSED);
    ...
    function testSendInstructionsRevertCosignersThresholdTooHigh() public {
        testRegister();
        vm.expectRevert(ITeeExtensionRegistryFacet.CosignersThresholdTooHigh.selector);
        flareTeeManager.sendInstructions(
            teeIds,
            ITeeExtensionRegistryFacet.TeeInstructionParams(
    ...
    function testSendInstructions() public {
        ...
        teeMachines[0] = ITeeMachineRegistryFacet.TeeMachine(
            teeIds[0],
            teeIds[0],
            url
        );
        teeMachines[1] = ITeeMachineRegistryFacet.TeeMachine(
            teeIds[1],
            teeIds[1],
            url
        );
    ...
    function testRegister() public {
        vm.prank(owner);
        vm.expectEmit();
        emit ITeeExtensionRegistryFacet.TeeExtensionRegistered(extensionId, owner);
    ...
    function testAddTeeVersionRevertVersionEmpty() public {
        testRegister();
        vm.prank(owner);
        vm.expectRevert(ITeeExtensionRegistryFacet.VersionEmpty.selector);
    ...
    function testAddTeeVersionRevertCodeHashZero() public {
        testRegister();
        vm.prank(owner);
        vm.expectRevert(ITeeExtensionRegistryFacet.CodeHashZero.selector);
    ...
    function testAddTeeVersionRevertUnsupportedPlatform() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ITeeExtensionRegistryFacet.UnsupportedPlatform.selector,
                platforms[0]
            )
        );
        flareTeeManager.addTeeVersion(extensionId, version, codeHash, platforms, governanceHash);
    ...
    function testAddTeeVersionRevertPlatformAlreadyExists() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ITeeExtensionRegistryFacet.PlatformAlreadyExists.selector,
                platforms[1]
            )
        );
        flareTeeManager.addTeeVersion(extensionId, version, codeHash, platforms, governanceHash);
    ...
    function testAddTeeVersion() public {
        ...
        vm.prank(owner);
        vm.expectEmit();
        emit ITeeExtensionRegistryFacet.TeeVersionAdded(
    ...
    function testDisableCodeHashPlatformRevertInvalidCodeHash() public {
        testRegister();
        vm.prank(owner);
        vm.expectRevert(ITeeExtensionRegistryFacet.InvalidCodeHash.selector);
    ...
    function testAddSystemSupportedKeyTypesAndSigningAlgosRevertNoSigningAlgos() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ITeeExtensionRegistryFacet.NoSigningAlgos.selector,
                keyTypes[0]
            )
        );
        flareTeeManager.addSystemSupportedKeyTypesAndSigningAlgos(keyTypes, signingAlgosByKeyType);
    ...
    function testAddSystemSupportedKeyTypesAndSigningAlgosRevertSigningAlgoEmpty() public {
        ...
        vm.prank(initialGovernance);
        vm.expectRevert(ITeeExtensionRegistryFacet.SigningAlgoEmpty.selector);
        flareTeeManager.addSystemSupportedKeyTypesAndSigningAlgos(keyTypes, signingAlgosByKeyType);
    }
    ...
    function testAddSystemSupportedKeyTypesAndSigningAlgosRevertSigningAlgoAlreadyExists() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ITeeExtensionRegistryFacet.SigningAlgoAlreadyExists.selector,
                keyTypes[1],
    ...
    function testAddSystemSupportedKeyTypesAndSigningAlgos() public {
        vm.prank(initialGovernance);
        vm.expectEmit();
        emit ITeeExtensionRegistryFacet.SystemSupportedKeyTypesAndSigningAlgosAdded(keyTypes, signingAlgosByKeyType);
        flareTeeManager.addSystemSupportedKeyTypesAndSigningAlgos(keyTypes, signingAlgosByKeyType);
    }
    ...
    function testAddSupportedKeyTypesRevertKeyTypeAlreadyExists() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ITeeExtensionRegistryFacet.KeyTypeAlreadyExists.selector,
                keyType
            )
        );
        flareTeeManager.addSupportedKeyTypes(extensionId, keyTypes);
    ...
    function testRemoveSupportedKeyTypes() public {
        ...
        vm.prank(owner);
        vm.expectEmit();
        emit ITeeExtensionRegistryFacet.SupportedKeyTypesRemoved(extensionId, keyTypes);
        flareTeeManager.removeSupportedKeyTypes(extensionId, keyTypes);
    }
    ...
    function testProposeNewOwner() public {
        ...
        vm.prank(owner);
        vm.expectEmit();
        emit ITeeExtensionRegistryFacet.NewOwnerProposed(extensionId, owner, newOwner);
        ITeeExtensionRegistryFacet(address(flareTeeManager)).proposeNewOwner(extensionId, newOwner);
    }
    ...
    function testConfirmOwnership() public {
        ...
        vm.prank(newOwner);
        vm.expectEmit();
        emit ITeeExtensionRegistryFacet.NewOwnerConfirmed(extensionId, newOwner);
        flareTeeManager.confirmOwnership(extensionId);
    ...
    function testAddSystemSupportedPlatforms() public {
        vm.prank(initialGovernance);
        vm.expectEmit();
        emit ITeeExtensionRegistryFacet.SystemSupportedPlatformsAdded(platforms);
        flareTeeManager.addSystemSupportedPlatforms(platforms);
    ...
    function testSendSystemInstructions() public {
        ...
        flareTeeManager.sendSystemInstructions(
            instructionId,
            teeIds,
            ITeeExtensionRegistryFacet.TeeInstructionParams(opType, opCommand, message, cosigners, 1, address(0))
        );
    }
    ...
    function _setupTeeGovernanceHash(
        ...
        // Set the governance hash through the TeeGovernanceFacet
