...
contract TeeReplicationFacetTest is Test {
    ...
    function testReplicateFromRevertExtensionMismatch() public {
        ...
        vm.prank(owner);
        vm.expectRevert(ITeeReplicationFacet.ExtensionMismatch.selector);
        flareTeeManager.replicateFrom{value: 200}(teeId, proof, teeUpgradeId, address(0));
    }
    ...
    function testReplicateFromRevertInvalidUpgradePath() public {
        ...
        ITeeVersionManagerFacet.TeeUpgradePath[] memory wrongPaths =
            new ITeeVersionManagerFacet.TeeUpgradePath[](1);
        ITeeVersionManagerFacet.TeeNodeVersion[] memory wrongSource =
            new ITeeVersionManagerFacet.TeeNodeVersion[](1);
        wrongSource[0] = ITeeVersionManagerFacet.TeeNodeVersion(codeHash2, platforms1[0]);
        ITeeVersionManagerFacet.TeeNodeVersion[] memory wrongTarget =
            new ITeeVersionManagerFacet.TeeNodeVersion[](1);
        wrongTarget[0] = ITeeVersionManagerFacet.TeeNodeVersion(codeHash1, platforms1[0]);
        wrongPaths[0] = ITeeVersionManagerFacet.TeeUpgradePath(wrongSource, wrongTarget);
        vm.prank(extensionOwner);
        ...
        vm.warp(block.timestamp + 1);
        vm.prank(owner);
        vm.expectRevert(ITeeReplicationFacet.InvalidUpgradePath.selector);
        flareTeeManager.replicateFrom{value: 200}(teeId, proof, teeUpgradeId, address(0));
    }
    ...
    function testReplicateFromRevertTeeUpgradeNotSigned() public {
        ...
        vm.prank(owner);
        vm.expectRevert(ITeeReplicationFacet.TeeUpgradeNotSigned.selector);
        flareTeeManager.replicateFrom{value: 200}(teeId, proof, teeUpgradeId, address(0));
    }
    ...
    function testReplicateFrom() public {
        ...
        vm.prank(owner);
        vm.expectEmit();
        emit ITeeReplicationFacet.TeeMachineReplicationTriggered(teeId, newTeeId, teeUpgradeId);
        flareTeeManager.replicateFrom{value: 200}(teeId, proof, teeUpgradeId, address(0));
    }
    ...
    function _addUpgradePaths(uint256 _upgradeId) private {
        ...
        ITeeVersionManagerFacet.TeeNodeVersion[] memory targetVersions =
            new ITeeVersionManagerFacet.TeeNodeVersion[](1);
        targetVersions[0] = ITeeVersionManagerFacet.TeeNodeVersion(codeHash2, platforms1[0]);
