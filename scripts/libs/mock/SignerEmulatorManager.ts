...
export class SignerEmulatorManager {
  ...
  public async run() {
    ...
    while (true) {
      ...
      const merkleRoots = Array.from({ length: this.signerEmulationConfig.numberOfSubProtocols }, () =>
        this.web3.utils.randomHex(32)
      );
      await Promise.all(
        signerEmulators.map(async (signerEmulator, index) => {
          // variance at sending
          const messages = this.randomMessages(
            merkleRoots,
            signingVotingRoundId,
            this.signerEmulationConfig.shareOfSignedSubprotocols
          );
