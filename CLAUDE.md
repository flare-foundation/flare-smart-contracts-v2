...
- **Optimizer**: enabled, 200 runs, `via_ir = true` — needed to keep `TeePaymentsUtxo` under the contract size limit. RNat, OZ's `P256`, `NodePossessionVerifier` and the mocks are pinned back to `via_ir = false` by `compilation_restrictions` (Yul stack / unimplemented-feature issues) — except `ERC20Mock` and `RelayMainDeployed`, which the Relay unit tests import: pinning them would drag `Relay`/`RelayProxy` and the FV suites into a second, non-viaIR compilation job; `[profile.coverage]` inverts this, since coverage forces viaIR off and a few contracts only compile with it. The restrictions also pin modern Forge sources to 0.8.35 and isolate the legacy 0.7.6/0.6.12 roots so compiler selection is deterministic on a clean machine.
