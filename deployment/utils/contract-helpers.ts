...
export async function executeTimelockedGovernanceCall(
  artifacts: unknown,
  contract: Truffle.ContractInstance,
  methodCall: (governance: string) => Promise<Truffle.TransactionResponse<Truffle.AnyEvent>>
) {
  ...
  const governanceSettings = (await (GovernanceSettings as { at: (address: string) => Promise<unknown> }).at(
    await contractGoverned.governanceSettings()
  )) as {
  ...
  const timelockArgs = findRequiredEvent(response, "GovernanceCallTimelocked").args as {
...
export async function testDeployGovernanceSettings(
  artifacts: unknown,
  governance: string,
  timelock: number,
  executors: string[],
  ...
  const provider = (network as { provider: { send: (method: string, params?: unknown[]) => Promise<unknown> } })
    .provider;
