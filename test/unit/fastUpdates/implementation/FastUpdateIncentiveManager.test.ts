...
contract(`FastUpdateIncentiveManager.sol; ${getTestFile(__filename)}`, (accounts) => {
  ...
  it("should get precision", async () => {
    ...
    expect(precision).to.equal((BigInt(RangeOrSampleFPA(RANGE)) << 127n) / BigInt(RangeOrSampleFPA(SAMPLE_SIZE)));
  });
  ...
  it("should offer incentive", async () => {
    ...
    const newRange = await fastUpdateIncentiveManager.getRange();
    expect(newRange).to.equal(BigInt(RangeOrSampleFPA(RANGE * 2)));

    const newSampleSize = await fastUpdateIncentiveManager.getExpectedSampleSize();
    expect(newSampleSize).to.equal(BigInt(RangeOrSampleFPA(SAMPLE_SIZE)));

    const precision = await fastUpdateIncentiveManager.getPrecision();
    expect(precision).to.equal((BigInt(newRange.toString()) << 127n) / BigInt(newSampleSize.toString()));

    const scale = await fastUpdateIncentiveManager.getScale();
    expect(scale).to.equal((1n << 127n) + (BigInt(newRange.toString()) << 127n) / BigInt(newSampleSize.toString()));
    const baseScale = await fastUpdateIncentiveManager.getBaseScale();
    expect(baseScale).to.equal(
      (1n << 127n) + (BigInt(RangeOrSampleFPA(RANGE)) << 127n) / BigInt(RangeOrSampleFPA(SAMPLE_SIZE))
    );
  ...
  it("should offer incentive and not increase range", async () => {
    ...
    const oldRange = await fastUpdateIncentiveManager.getRange();
  ...
  it("should offer incentive and only increase sample size", async () => {
    ...
    const oldRange = await fastUpdateIncentiveManager.getRange();
    expect(oldRange).to.equal(RangeOrSampleFPA(RANGE));
    const oldSampleSize = await fastUpdateIncentiveManager.getExpectedSampleSize();
  ...
  it("should offer incentive but get everything back 2", async () => {
    ...
    expect((result.receipt as { gasUsed: number }).gasUsed * +tr.gasPrice).to.equal(
      BigInt(oldBalance) - BigInt(newBalance)
    );
  ...
  it("should revert offering incentive if contribution is insufficient", async () => {
    ...
    if (!accounts[1]) throw new Error("Account not found");
    await expectRevert(
      fastUpdateIncentiveManager.offerIncentive(offer, { from: accounts[1] }),
  ...
  it("should revert if the parameters would allow making the precision greater than 100%", async () => {
    ...
    await expectRevert(
      fastUpdateIncentiveManager.setRangeIncreaseLimit(RangeOrSampleFPA(4), {
        from: governance,
      }),
      "Parameters should not allow making the precision greater than 100%"
    );
  ...
  it("should change incentive duration", async () => {
    ...
    await fastUpdateIncentiveManager.setIncentiveParameters(
      RangeOrSampleFPA(SAMPLE_SIZE),
      RangeOrSampleFPA(RANGE),
      SAMPLE_SIZE_INCREASE_PRICE,
      10,
    ...
    const newIncentiveDuration = await fastUpdateIncentiveManager.getIncentiveDuration();
  ...
  it("should revert if setting circular length to zero", async () => {
    await expectRevert(
      fastUpdateIncentiveManager.setIncentiveParameters(
        RangeOrSampleFPA(SAMPLE_SIZE),
        RangeOrSampleFPA(RANGE),
        SAMPLE_SIZE_INCREASE_PRICE,
        0,
      ...
      "CircularListManager: circular length must be greater than 0"
    );
  });
  ...
  it("Should trigger inflation offers", async () => {
    ...
    const trigger = await fastUpdateIncentiveManager.triggerRewardEpochSwitchover(2, 3 * DAY + time, DAY, {
      from: flareSystemManager,
    });
  ...
  it("Should set price increase range and sample increase limit", async () => {
    ...
    // change values
    await fastUpdateIncentiveManager.setSampleIncreaseLimit(
      BigInt(RangeOrSampleFPA(SAMPLE_INCREASE_LIMIT * 2)).toString(),
      { from: governance }
    );
    await fastUpdateIncentiveManager.setRangeIncreaseLimit(
      BigInt(RangeOrSampleFPA(RANGE_INCREASE_LIMIT * 2)).toString(),
      { from: governance }
    );
    await fastUpdateIncentiveManager.setRangeIncreasePrice((RANGE_INCREASE_PRICE * BigInt(2)).toString(), {
      from: governance,
    });

    expect(await fastUpdateIncentiveManager.sampleIncreaseLimit()).to.equal(
      BigInt(RangeOrSampleFPA(SAMPLE_INCREASE_LIMIT * 2))
    );
    expect(await fastUpdateIncentiveManager.rangeIncreaseLimit()).to.equal(
      BigInt(RangeOrSampleFPA(RANGE_INCREASE_LIMIT * 2))
    );
  ...
  it("Should set sample size and range", async () => {
    ...
    // change values
    await fastUpdateIncentiveManager.setIncentiveParameters(
      RangeOrSampleFPA(SAMPLE_SIZE * 2),
      RangeOrSampleFPA(RANGE * 2),
      SAMPLE_SIZE_INCREASE_PRICE * 2,
      DURATION,
      { from: governance }
    );
    ...
    expect(await fastUpdateIncentiveManager.getCurrentSampleSizeIncreasePrice()).to.equal(
      SAMPLE_SIZE_INCREASE_PRICE * 2
    );
  });
  ...
  it("should revert when setting sample increase limit or range increase price if value too big", async () => {
    ...
    await expectRevert(
      fastUpdateIncentiveManager.setSampleIncreaseLimit(value, { from: governance }),
      "Sample increase limit too large"
    );

    await expectRevert(
      fastUpdateIncentiveManager.setRangeIncreasePrice(value, { from: governance }),
      "Range increase price too large"
    );
  });
  ...
  it("should revert when setting sample size, range etc. if value too big/small", async () => {
    ...
    await expectRevert(
      fastUpdateIncentiveManager.setIncentiveParameters(
        RangeOrSampleFPA(RANGE_INCREASE_LIMIT * 2),
        RangeOrSampleFPA(RANGE_INCREASE_LIMIT * 3),
        SAMPLE_SIZE_INCREASE_PRICE,
        DURATION,
        { from: governance }
      ),
      "Range cannot be greater than the range increase limit"
    );

    await expectRevert(
      fastUpdateIncentiveManager.setIncentiveParameters(
        RangeOrSampleFPA(1),
        RangeOrSampleFPA(2 ** -30),
        SAMPLE_SIZE_INCREASE_PRICE,
        DURATION,
        { from: governance }
      ),
  ...
  it("should revert if not setting base sample size, base range etc. from governance", async () => {
    ...
    await expectRevert(
      fastUpdateIncentiveManager.setSampleIncreaseLimit(RangeOrSampleFPA(SAMPLE_INCREASE_LIMIT), { from: accounts[1] }),
      "only governance"
    );

    await expectRevert(
      fastUpdateIncentiveManager.setRangeIncreasePrice(RANGE_INCREASE_PRICE.toString(), { from: accounts[1] }),
      "only governance"
    );

    await expectRevert(
      fastUpdateIncentiveManager.setRangeIncreaseLimit(RangeOrSampleFPA(RANGE_INCREASE_LIMIT), { from: accounts[1] }),
      "only governance"
    );
  });
  ...
  it("should revert when setting base range, range increase price or range increase limit too low", async () => {
    ...
    await expectRevert(
      fastUpdateIncentiveManager.setRangeIncreasePrice(5, { from: governance }),
      "Range increase price too low, range increase of 1e-6 of base range should cost at least 1 wei"
    );

    await expectRevert(
      fastUpdateIncentiveManager.setRangeIncreaseLimit(5, { from: governance }),
      "Range cannot be greater than the range increase limit"
    );
  });
});
