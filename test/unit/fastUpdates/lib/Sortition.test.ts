...
contract(`Sortition.sol; ${getTestFile(__filename)}`, (accounts) => {
  ...
  before(async () => {
    ...
    sortition = await SortitionContract.new(governance as Truffle.TransactionDetails);
  });
  ...
  it("should generate a verifiable randomness", async () => {
    ...
    const proof: Proof = generateVerifiableRandomnessProof(key, seed, blockNum, replicate);
    ...
    const check = await sortition.verifySortitionProofTest(sortitionState, sortitionCredential);
  ...
  it("should correctly accept or reject the randomness", async () => {
    ...
    for (;;) {
      ...
      const proof: Proof = generateVerifiableRandomnessProof(key, seed, blockNum, replicate.toString());
      const pubKey = {
        x: key.pk.x.toString(),
        y: key.pk.y.toString(),
      };
      ...
      const check = await sortition.verifySortitionCredentialTest(sortitionState, sortitionCredential);
