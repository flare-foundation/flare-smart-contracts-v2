...
contract(`FixedPointArithmetic.sol; ${getTestFile(__filename)}}`, (accounts) => {
  ...
  before(async () => {
    ...
    fpaInstance = await TestFixedPointArithmetic.new(governance as Truffle.TransactionDetails);
  });
  ...
  it("should add and subtract SampleSize values", async () => {
    ...
    const c = await fpaInstance.addSampleSizeTest(x.toString(), y.toString());
