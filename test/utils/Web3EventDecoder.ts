...
export type TruffleExtractEvent<E extends Truffle.AnyEvent, N extends E['name']> = Truffle.TransactionLog<Extract<E, { name: N }>>;

export type ContractWithEvents<C extends Truffle.ContractInstance, E extends Truffle.AnyEvent> = C & { '~eventMarker'?: E };
...
class Web3EventDecoder {
    ...
    findEvent<E extends Truffle.AnyEvent, N extends E['name']>(response: Truffle.TransactionResponse<E>, name: N): TruffleExtractEvent<E, N> | undefined {
        const logs = this.decodeEvents(response);
        return logs.find(e => e.event === name) as any;
    }
}

export function findRequiredEventFrom<C extends Truffle.ContractInstance, E extends Truffle.AnyEvent, N extends E['name']>(response: Truffle.TransactionResponse<any>, contract: ContractWithEvents<C, E>, name: N): TruffleExtractEvent<E, N> {
...
export function requiredEventArgsFrom<C extends Truffle.ContractInstance, E extends Truffle.AnyEvent, N extends E['name']>(response: Truffle.TransactionResponse<any>, contract: ContractWithEvents<C, E>, name: N): TruffleExtractEvent<E, N>['args'] {
