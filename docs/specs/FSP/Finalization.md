...
2. **Protocol message** (38 bytes) — `protocolId` (1) + `votingRoundId` (4) + `isSecureRandom` (1, **bool**) + `merkleRoot` (32). `isSecureRandom` is a boolean encoded in one byte and carries meaning only for the random-number protocol: Relay requires it to be `0` or `1` for `protocolId == randomNumberProtocolId` and exactly `0` for every other protocol id, `protocolId == 1` included, rejecting anything else with `WrongMessageFormat2` while parsing the message. No downstream path normalizes it, so the value reaching the random leaf, the stored quality bit and the `bool` field of `ProtocolMessageRelayed` is exactly the signed one. The digest this message is signed under is the analogous single keccak, `keccak256(sourceChainId ‖ protocolMessage)` (before the EIP-191 `"\x19Ethereum Signed Message:\n32"` prefix applied at signature verification).
...
For the FTSO random-number protocol (`stateData.randomNumberProtocolId`), calldata after the signatures contains a 32-byte random value followed by zero or more 32-byte Merkle-proof nodes. Relay proves `keccak256(abi.encode(votingRoundId, randomNumber, isSecureRandom))` under the signed root, taking `votingRoundId` and `isSecureRandom` from the signed message and only the value from the trailer, stores the value and per-round secure bit, and advances the live pointer only when `votingRoundId` is strictly greater than the current pointer. See [Random Number](./RandomNumber.md).
