...
4. **Signing policy sign phase** — the *previous* reward epoch's voters call `signNewSigningPolicy`. When accumulated weight passes the previous policy's threshold, `signingPolicySignEndTs/Block` is set. This timestamp drives late-signing penalties and rewards-signing eligibility; it is not a reward-epoch-start gate.
5. **Reward epoch start** — once wall-clock time reaches `currentRewardEpochExpectedEndTs`, the policy hash is initialized, and the current voting round reaches the policy's `startVotingRoundId`, the orchestrator advances `currentRewardEpochExpectedEndTs`, sets `rewardEpochStartTs/Block`, and emits `RewardEpochStarted`. `_getStartVotingRoundId` fixes the start round at policy initialization using the scheduled boundary and the minimum round-delay rule; the start does not wait for signature threshold completion.
