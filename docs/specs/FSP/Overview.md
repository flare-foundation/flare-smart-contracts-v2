...
| [`Relay`](../../../contracts/protocol/implementation/Relay.sol) | The finalization sink. Verifies threshold signatures of the active signing policy over a `(protocolId, votingRoundId, merkleRoot)` tuple and emits `ProtocolMessageRelayed`. Also publishes signing policies and the FSP random number. |
...
5. The *current* reward epoch's voters then enter the **sign phase**, threshold-signing the new policy. Once threshold is reached, the new epoch officially begins.
...
The pre-v2 specs reference a "Flare Daemon" contract that triggers per-block lifecycle hooks. In this repo the daemon-driven behavior is invoked through `FlareSystemsManager.daemonize()` and analogous hooks on other contracts; the daemon itself is part of the validator runtime, not a contract in this repo. See [Architecture / The orchestrator](../Architecture.md#the-orchestrator-flaresystemsmanager).
