...
The two systems can run side-by-side; they share the [`(attestationType, sourceId)` taxonomy](./AttestationTypes.md). Each generation has its own inflation pool: legacy FDC routes inflation through [`FdcHub`](../../../contracts/fdc/implementation/FdcHub.sol) + [`FdcInflationConfigurations`](../../../contracts/fdc/implementation/FdcInflationConfigurations.sol), while FDC2 uses a standalone [`Fdc2RewardOffersManager`](../../../contracts/fdc2/implementation/Fdc2RewardOffersManager.sol) + [`Fdc2InflationConfigurations`](../../../contracts/fdc2/implementation/Fdc2InflationConfigurations.sol). User-paid request fees for both are forwarded to `RewardManager.receiveRewards` under the current reward epoch (`Fdc2Hub` does this directly).
