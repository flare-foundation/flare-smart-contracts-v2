...
- **TEE-machine verification.** `verifyTeeSignature(sig, hash)` and `verifyTeeSignatures(sigs[], hash)` recover the TEE machine's address via ECDSA, then check `flareTeeManager.getExtensionId(teeId) == 0` (the system extension) and `flareTeeManager.getTeeMachineStatus(teeId) == PRODUCTION`. Used for direct on-Flare verification. Both also have overloads taking the extension id as the first parameter — `verifyTeeSignature(extensionId, sig, hash)` and `verifyTeeSignatures(extensionId, sigs[], hash)` — with the same acceptance rule applied against a caller-chosen extension (emergency pause included, reverting with `ExtensionEmergencyPaused`); non-system TEE extensions (for example a TEE oracle on a reserved extension id) reuse these instead of replicating the check.
...
Cosigner signatures use a separate preimage wrap [`Fdc2ProofVerification.toCosignersMessageHash`](../../../contracts/fdc2/library/Fdc2ProofVerification.sol) — `keccak256(bytes32(block.chainid) || 0x010000000000 || messageHash)`. This is byte-for-byte the [`Relay`](../../../contracts/protocol/implementation/Relay.sol) Mode-2 signed digest for the protocol message `(protocolId=1, votingRoundId=0, isSecureRandom=false, merkleRoot=messageHash)` on a home deployment (where the Relay's `sourceChainId` equals `block.chainid`); keeping the shapes identical makes cosigner signatures and relay/signing-policy signatures over the same message interchangeable — one signature set verifies both via `recoverCosigners` and via `IRelay.verifyCustomSignature`, so a cosigner who is also a signing-policy data signer uses the same off-chain signing infrastructure with no special handling.

> **Cutover note.** All TEE machines require redeployment when the digest scheme changes: they verify the data-provider signatures inside the enclave before adding their own signature, so an old binary — computing the previous digest — rejects every new single-keccak signature. On-chain consumers split in two: those that only delegate verification to the Relay (`verifySigningPolicySignatures` → `IRelay.verifyCustomSignature`, and `recoverCosigners`, which receives the preimage as a parameter) need no bytecode change, while those that inline `Fdc2ProofVerification.toCosignersMessageHash` — an `internal` library function compiled into its callers: the TEE Diamond facets via the [`Verification`](../../../contracts/tee/library/Verification.sol) library and [`TeePaymentsConfigVerifier`](../../../contracts/tee/implementation/TeePaymentsConfigVerifier.sol) — need new deployed bytecode (facet replacement via `diamondCut` / UUPS upgrade). The cutover batch also includes repointing `Fdc2Verification`'s `relay` reference to the new Relay, and it deliberately invalidates any existing proof whose signatures were minted under the previous digest — such proofs must be regenerated. See `docs/relay-governance.md` §6 for the full simultaneous checklist.
