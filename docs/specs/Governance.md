...
Value handling in the new stack: `executeGovernanceCall` is `payable` and forwards its entire attached value into the executed call, so `payable onlyGovernance` methods can receive funds at execution time — the value is not part of the stored call hash, so the amount is decided when executing (a non-payable target rejects attached value, reverting the execution and keeping the call pending). Recording a timelocked call rejects any attached value (`TimelockValueNotAllowed`) because the body does not run at recording time and the value would be trapped — the same guard `OwnableWithTimelock` applies when queuing.

Because the attached value is chosen at execution time, it is the one input to a timelocked call that escapes the timelock's public-commitment guarantee: observers see the exact calldata for the whole timelock window, but not the value that will accompany it. This is acceptable only as long as the value is pure fee funding — needed because dispatch fees (per-operation fee × number of target machines, both resolved at execution) can change between queuing and the ETA, and pre-funding would leave native balance on the contract. **Design rule:** a `payable onlyGovernance` method must treat `msg.value` strictly as fee funding — it must never branch on it, store it, or let it select behavior. A method that did would hand the executor an untimelocked governance parameter. (`OwnableWithTimelock` deliberately keeps the stricter contract: `executeTimelockedCall` is non-payable and always replays with zero value, so that timelock retains the full commit-then-execute guarantee.)
