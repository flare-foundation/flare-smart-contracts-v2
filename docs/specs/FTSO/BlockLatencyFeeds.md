...
- Each provider has a **sortition public key** (a `G1Point` on the alt-bn128 curve) registered through [`EntityManager.registerPublicKey`](../../../contracts/protocol/implementation/EntityManager.sol). `FastUpdater` itself implements `IIPublicKeyVerifier` — `verifyPublicKey(voter, p1, p2, verificationData)` is the verifier installed in `EntityManager` for FTSO-purpose public keys, and uses BLS-style key-of-knowledge verification (`Sortition.verifySignature(pk, sha256(voter), σ, R)`).
...
1. Looks up the per-feed read fee through [`FeeCalculator`](../../../contracts/userInterfaces/IFeeCalculator.sol) and requires `msg.value` to cover it. It forwards exactly the calculated fee to `feeDestination`; direct overpayment is not refunded and remains on `FastUpdater`. FtsoV2 calculates and checks each downstream fee before forwarding it. If the value remaining from the current call is below that fee, it reverts with `"too low fee"` before calling that downstream source. FtsoV2 forwards only the required amount and burns the current caller's excess after a successful read; any pre-existing balance remains untouched. Addresses on the governance-set `freeFetchAddresses` allowlist must send zero value. The allowlist is checked against the immediate `FastUpdater` caller — for an FtsoV2-routed read, that is `FtsoV2` or a custom-feed contract, not the end user. Consequently, neither FtsoV2 nor a custom feed that forwards value to FastUpdater may be added to this allowlist.
