...
**`reissue(account, paymentId, [paymentInstruction], reissueFeeParams, startNew, claimBackAddress)`** re-sends exactly one previously-paid payment (e.g. with a higher max fee). It requires `startNew == true` (reverts `StartNewRequired` otherwise — the account model has no multi-call replacement to continue, so every reissue is a fresh, standalone one; the shared flag exists for the UTXO model) and `paymentInstructions.length == 1`, verifies the supplied instruction against the stored `paymentHashes[accountHash][paymentId]` **before** any external calls (cheap revert), bumps a per-`(account, paymentId)` `reissueCounter` to a `reissueNumber` (starting at 1; PAY uses 0), and dispatches with op command `"REISSUE"` and instruction id `_computeInstructionId(opType, "REISSUE", sourceId, accountAddress, paymentId, reissueNumber)`. It always returns `true` — the account model reissues a single payment in one instruction, so it is finalized immediately.
...
`reissue(account, batchPaymentId, paymentInstructions, reissueFeeParams, startNew, claimBackAddress)` re-sends the payments of an already-closed batch (e.g. to bump fees, or to nullify with a negative fee factor). The batch must have ended.
...
- The `startNew` flag chooses between starting and continuing — explicitly, not inferred from the payment content (so a batch of identical payments continues correctly). `startNew == true` **starts a fresh attempt** (`++id`, reset `nextPaymentId` to `batchPaymentId`, reset `emittedCount`/`blocks`), emitting `UtxoReplacementStarted`; it overwrites any active attempt, so it is also how a caller deliberately restarts from the top (a reset reward epoch likewise requires `startNew == true`). `startNew == false` **continues** the active attempt from its stored `nextPaymentId` — reverting `NoActiveReplacement` if none is active/non-finalized, or `ReissueRewardEpochChanged` if the epoch has rolled. A large batch is reissued across multiple calls: one `startNew == true` followed by `startNew == false` continuations; use `getBatchRecord` / the replacement's `nextPaymentId` to resume after a restart of the off-chain driver.
