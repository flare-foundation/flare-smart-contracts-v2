...
The public `FtsoV2` reader (UUPS proxy) at [`FtsoV2Interface`](../../contracts/userInterfaces/LTS/FtsoV2Interface.sol) is the recommended entry point for application contracts. Batched reads come in two flavors: `getFeedsById` / `getFeedsByIdInWei` return one shared timestamp (reverting if the feeds report different timestamps), while `getCurrentFeeds` / `getCurrentFeedsInWei` return a timestamp per feed. Both families revert if the list is empty. The `getCurrentFeed(s)` family (single and batched, plain and wei) returns signed values; custom feeds may report negative ones, while the unsigned `getFeedsById` family reverts `"value negative"`. Wei conversion panics with arithmetic overflow if its power-of-ten factor or scaled-up result would not fit in the return type (`int256` for signed reads or `uint256` for unsigned reads). See [FTSO / Overview](./FTSO/Overview.md).
...
| [`IFdc2Verification`](../../contracts/userInterfaces/fdc2/IFdc2Verification.sol) | `verifySigningPolicySignatures`, `verifyTeeSignature(s)` (system extension, plus per-extension-id overloads), `recoverCosigners`. |
