...
| **BR-3 / K-2** (encoding fidelity, model↔bytecode) | Addressable | **largely closed** — `RelayBodyEff.bodyL` is the deployed body transliterated statement-for-statement (D1–D4 deviations); early-return now **modeled faithfully** (`relay_loop_sound_literal_early`, accept → `return(0,0)`), leaving only the registered D-deviations (folded addressing, revert/accept payloads → `revert/return(0,0)`, loop-invariant scalars as params) as departures — of which D3 (accept → `return` vs. break→write→return) is the one the whole-`relay()` composition still keeps the accept-*write* separate over |
...
- **What *is* claimed, fully:** the security-critical accounting is sound for all N and all K (rows 1, 1c,
  ...
  randomness, and fees hold on the real bytecode within bound (rows 2–14); the whole-`relay()` extension — mode
  dispatch, accept-write, fee conservation, and the dispatch→loop→accept composition — is machine-checked
  hole-free (R5, §10.5), adding breadth over the core loop soundness; and the gap to the literal
