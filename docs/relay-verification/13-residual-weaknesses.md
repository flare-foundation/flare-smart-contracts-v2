...
**T1-b · `verifyCustomSignature()` is an unbound public signature oracle** — `Relay.sol:465-470` (review item **L-3**). *Class: narrowed by RLY-23; residual is now same-chain-only.* Since **RLY-23** the verified digest is chain-bound (`keccak256(sourceChainId ‖ H)` is what voters sign), so the **cross-chain** half of this replay surface is closed on-chain: the same quorum signatures are inert against an identical-policy Relay on a *different* network. What remains is same-chain / cross-*deployment*: `verifyCustomSignature` still forwards the caller's raw `_messageHash` with no *contract-address* or *nonce* binding, so two Relays on one chain with an overlapping policy can still replay a third party's message. The residual mitigation is the (updated) NatSpec "callers MUST domain-separate `_messageHash` with the consuming contract and a nonce" note — a smaller, same-chain surface than before, still exported to third parties the Relay team cannot control. (The broader cross-chain replay of the *core* `relay()` policy-rotation and Merkle/random paths — never a filed finding, adjacent to this one — is likewise closed by RLY-23; see `docs/relay-fixes.md` RLY-23.)
